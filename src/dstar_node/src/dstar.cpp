...
DStar::DStar(const costmap_t costmap)
{
    costmap_ = costmap;
...
void DStar::updateStart(unsigned int x, unsigned int y)
{
  vertex *new_start = &vertex_map[x][y];

  km += heuristic(start_, new_start);
  start_ = new_start;
}
...
bool DStar::updateCost(unsigned int x, unsigned int y, unsigned char cost)
{
